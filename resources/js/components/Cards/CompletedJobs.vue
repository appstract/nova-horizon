<template>
    <card class="nova-horizon flex flex-col">
        ...
        <div v-if="ready && jobs.length" class="p-3 flex justify-between border-t border-gray-200">
...
export default {
    ...
    methods: {
        ...
        loadJobs(starting = -1, refreshing = false) {
            ...
            Nova.request().get(config.novaHorizon.basePath + '/api/jobs/completed?starting_at=' + starting + '&limit=' + this.perPage)
                .then(response => {
                    if (
                        refreshing &&
                        this.jobs.length &&
